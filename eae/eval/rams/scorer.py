"""Official RAMS scorer.

The only edits made are to the imports.
"""
import argparse
import json
import re
from collections import Counter, defaultdict
from eae.eval.rams.constraints import Constraints
import eae.eval.rams.scoring_utils as util
...
class Scorer(object):
    ...
    def read_gold_file(self, file_path, confidence=False):
        ...
        def process_example(json_blob):
            ...
            evt_trigger_types = set(
                [evt_trigger_type[0] for evt_trigger_type in evt_trigger[2]]
            )

            gold_evt_links = [
                (
                    util.list_to_span(arg[0]),
                    util.list_to_span(arg[1]),
                    self.get_role_label(arg[2]),
                )
                for arg in gold_evt
            ]
            if confidence:
                gold_evt_links = [(a, b, c, 0) for a, b, c in gold_evt_links]
            assert all([arg[0] == evt_trigger_span for arg in gold_evt_links])
        ...
        file_dict = {
            doc_key: (evt_links, evt_trigger_types, span_to_sent)
            for doc_key, evt_links, evt_trigger_types, span_to_sent in lines
        }
        return file_dict
    ...
    def read_preds_file(self, file_path):
        ...
        def process_example(json_blob):
            ...
            evt_args = pred_evt[1:]
            pred_args = [
                (evt_span, util.list_to_span(args[:2]), args[2], args[3])
        ...
        lines = [process_example(json.loads(line)) for line in jsonlines]
        file_dict = {
            doc_key: (evt_links, evt_trigger_types)
            for doc_key, evt_links, evt_trigger_types in lines
        }
        return file_dict
    ...
    def create_role_table(self, correct, missing, overpred):
        ...
        total_p, total_r, total_f1 = util.compute_metrics(total_c, total_m, total_o)
        totals = {
            "CORRECT": total_c,
            "MISSING": total_m,
            "OVERPRED": total_o,
    ...
    def evaluate(self, constrained_decoding=True):
        ...
        sentence_breakdowns = [
            {"correct": Counter(), "missing": Counter(), "overpred": Counter()}
            for i in range(5)
        ]
        total_lost = 0
        ...
        for doc_key, (gold_structure, evt_type, span_to_sent) in self.gold.items():
            ...
            pred_structure, lost = self.constraints.filter_preds(
                pred_structure, evt_type, constrained_decoding
            )
            ...
            util.update_sentence_breakdowns(
                intersection, missing, overpred, sentence_breakdowns, span_to_sent
            )
        precision, recall, f1, _ = util.compute_from_counters(
            global_correct, global_missing, global_overpred
        )
        distance_metrics = []
        for i in range(5):
        ...
        self.metrics = {"precision": precision, "recall": recall, "f1": f1}
        self.distance_metrics = distance_metrics
        self.role_table = self.create_role_table(
            global_correct, global_missing, global_overpred
        )
        return {
...
def run_evaluation(args):
    ...
    if args.distance or args.do_all:
        for i, (p, r, f1), (gold, pred) in return_dict["distance_metrics"]:
            print(
                " {} & {} & {:.1f} & {:.1f} & {:.1f} \\\\ [p r f1 {} gold/{} pred. ]".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--gold_file", type=str, help="Gold file path")
    parser.add_argument(
        "-p", "--pred_file", type=str, default=None, help="Predictions file path"
    )
    parser.add_argument(
        "--reuse_gold_format",
        dest="reuse_gold_format",
        default=False,
        action="store_true",
        help="Reuse gold file format for pred file.",
    )
    parser.add_argument(
        "-t", "--ontology_file", type=str, default=None, help="Path to ontology file"
    )
    parser.add_argument(
        "-cd",
