...
def update_sentence_breakdowns(
    intersection, missing, overpred, sentence_breakdowns, span_to_sent
):
    def split_to_sent(span_list):
        sent_idx = [
            span_to_sent(span[1]) - span_to_sent(span[0]) + 2 for span in span_list
        ]
        return [
            Counter(
                {
...
def compute_from_counters(correct, missing, overpred):
    ...
    precision, recall, f1 = compute_metrics(
        total_correct, total_missing, total_overpred
    )
...
def print_confusion(confusion):
    ...
    confusion_matrix = [
        ",".join(
            [r1]
            + [
                "{}".format(int(100 * confusion[r1][r2] / sum(confusion[r1].values())))
